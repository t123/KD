...
public partial class ComboBox<T> : MudBaseInputExtended<T>
{
    ...
    /// </summary>
    public List<ComboBoxItem<T>> Items { get; set; } = new();
    internal ComboBoxItem<T>? _lastActivatedItem;

    /// <summary>
    /// Eligible items of ComboBox.
    /// </summary>
    protected internal List<ComboBoxItem<T>> EligibleItems { get; set; } = new();
    ...
    public RenderFragment<ComboBoxItem<T>>? ItemSelectedTemplate { get; set; }
    ...
    [Parameter]
    [Category(CategoryTypes.FormComponent.ListAppearance)]
    public RenderFragment<ComboBoxItem<T>>? ItemDisabledTemplate { get; set; }
    ...
    /// <returns></returns>
    protected internal async Task ToggleOption(ComboBoxItem<T>? item, bool selected)
    {
        if (item == null)
        {
            return;
    ...
    /// <param name="item"></param>
    /// <returns></returns>
    protected internal bool? AddItem(ComboBoxItem<T> item)
    {
        if (item == null)
            return false;
        bool? result = null;
    ...
    /// </summary>
    /// <param name="item"></param>
    protected internal void RemoveItem(ComboBoxItem<T> item) => Items.Remove(item);
    ...
    /// </summary>
    /// <returns></returns>
    protected internal List<ComboBoxItem<T>> GetEnabledAndEligibleItems() => Items.Where(x => !x.Disabled && x.Eligible).ToList();
    ...
    /// <param name="item"></param>
    /// <returns></returns>
    protected internal ValueTask ScrollToMiddleAsync(ComboBoxItem<T> item) => item is not null ? ScrollManagerExtended.ScrollToMiddleAsync(_popoverId, item.ItemId) : ValueTask.CompletedTask;
